...
export default function Navbar() {
  ...
  return <motion.header initial={{ opacity: 0, y: -18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65, ease: 'easeOut' }} className={`fixed inset-x-0 top-0 z-40 border-b transition-all duration-500 ${scrolled ? 'border-white/10 bg-black/65 shadow-[0_12px_36px_rgba(0,0,0,.28)] backdrop-blur-xl' : 'border-white/10 bg-black/20 backdrop-blur-md'}`}>
    <nav className="mx-auto flex min-h-[68px] max-w-7xl items-center justify-between px-4 sm:min-h-[72px] sm:px-6 lg:px-8" aria-label="Navegação principal">
      <Logo />
      <button className="grid min-h-11 min-w-11 place-content-center gap-1.5 rounded-lg border border-white/10 bg-white/5 xl:hidden" onClick={() => setOpen(!open)} aria-label="Abrir menu" aria-expanded={open}><span className="h-0.5 w-6 bg-white" /><span className="h-0.5 w-6 bg-white" /><span className="h-0.5 w-6 bg-white" /></button>
    ...
    <AnimatePresence>{open && <motion.div initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 18 }} className="absolute right-0 top-full h-[calc(100vh-68px)] w-[min(86vw,340px)] overflow-y-auto border-l border-t border-white/10 bg-black/85 px-4 pb-5 pt-4 shadow-2xl backdrop-blur-xl xl:hidden">{links.map(([id, label]) => <button className="block min-h-12 w-full border-b border-white/5 text-left text-base text-slate-200" onClick={() => goTo(id)} key={id}>{label}</button>)}<a className="mt-5 flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#071a35] px-4 py-3 text-center text-sm" href={whatsapp} target="_blank" rel="noreferrer"><img className="h-5 w-5 object-contain" src="/imgs/whatsapp.png" alt="" />Falar no WhatsApp</a></motion.div>}</AnimatePresence>
  </motion.header>
}
